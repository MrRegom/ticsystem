...
def fix_layout(filepath):
    ...
    if extra_match:
        # Check if it contains the cards
        if 'Últimos Tickets Pendientes' in extra_match.group(0):
            html = html.replace(extra_match.group(0), '')
    else:
        # It's possible the regex didn't match the closing properly, let's just use string slicing
    ...
    act_match = re.search(r'<div class="fluent-card" style="margin-top: 24px;">\s*<div class="fluent-card-header">\s*<span>Actividad Reciente del Inventario</span>.*?</table>\s*</div>\s*</div>', html, re.DOTALL)
    if act_match:
        act_html = act_match.group(0)
        # make it full height
        act_html = act_html.replace('style="margin-top: 24px;"', 'style="height: 100%;"')
