...
# 2. Reasignar perfiles de roles a borrar hacia los nuevos roles oficiales
def migrar_perfiles(nombre_origen, rol_destino):
    try:
        rol_or = Rol.objects.get(nombre=nombre_origen)
        count = PerfilUsuario.objects.filter(rol=rol_or).update(rol=rol_destino)
        rol_or.delete()
        print(f"Rol '{nombre_origen}' eliminado. {count} perfiles migrados a '{rol_destino.nombre}'.")
    except Rol.DoesNotExist:
        pass

# Ejecutar migración y limpieza mediante PerfilUsuario
migrar_perfiles('Mesa de Ayuda', rol_operador)
migrar_perfiles('TEST ROL', rol_operador)
migrar_perfiles('Técnico de Soporte Nivel 2', rol_tecnico)
migrar_perfiles('Técnicos Terreno', rol_tecnico)
